...
namespace XNodeEditor
{
        ...
        NodeGraphEditor.CustomNodeGraphEditorAttribute, XNode.NodeGraph>
    {
            ...
            XNode.NodePort.IO direction = XNode.NodePort.IO.Input)
        {
            ...
            menu.AddItem(new GUIContent("创建节点"), false, () => { nodesCreator.ShowInPopup(mousePosition); });
            menu.AddSeparator("");
            ...
            NodePort.IO direction, Vector2 pos)
        {
            ...
            for (int i = 0; i < nodeTypes.Length; i++)
            {
                ...
                //排除掉特殊节点
                var customNodeTitleAttribute = type.GetCustomAttributes<Node.NodeTitleAttribute>().FirstOrDefault();
                
                if(customNodeTitleAttribute!=null && !customNodeTitleAttribute.allowCustomName)
                    continue;
            ...
            GenericSelector<Type> nodesCreator = new GenericSelector<Type>("选择节点", false,
                x =>
                {
                    //如果节点自定义了NodeTitle属性，就使用属性中的名字
                    var customNodeTitle = x.GetCustomAttributes<Node.NodeTitleAttribute>().FirstOrDefault();
                    if (customNodeTitle != null)
                    {
                        if (customNodeTitle.nodeType =="")
                        {
                            return $"TeamFlow/{customNodeTitle.title}";
                        }
                        return $"TeamFlow/{customNodeTitle.nodeType}/{customNodeTitle.title}";
                    }
                    return typesCache[x];
                }, typesCache.Keys);
